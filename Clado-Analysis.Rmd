...
# Summary of class abundance of classes_zulk of interest. 
relabund.classzulk.class.est.nosplit <- summarySE(relabund.classzulk.class.est, measurevar= "ClassAbundance", groupvars=c("Class"))
...
```
```{r plot_est_classes_zulk_all_nosplit, fig.width=5, fig.height=4}
df <- relabund.classzulk.class.est.merge.nosplit
limits <- aes(ymin=df$ClassAbundance-df$ci, ymax=df$ClassAbundance+df$ci)
p <- ggplot(df, aes(Class)) 
p + geom_point(aes(y = df$ClassAbundance, color = "Braus 2016 (2014)"), size = 3, alpha = 0.5) + 
  geom_errorbar(limits, color = "darkred") +
  geom_point(aes(y = df$ClassAbundance_zulkifly, color = "Zulkifly 2012 (2011)"), size = 3, alpha = 0.5) + 
