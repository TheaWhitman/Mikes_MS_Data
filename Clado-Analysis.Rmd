...
```
```{r plot_richness_sha, fig.height=2, fig.width=3}
...
p <- qplot(clado.depth.zoom$daynum, y = clado.depth.zoom$wtemp) + 
  ...
  labs(title = "Water Temperature <1.5m Depth, Lake Mendota", x="Date", y="Temperature °C") + 
  geom_vline(xintercept=Jdays, colour="darkgreen", linetype = "longdash") + 
...
p <- ggplot(clado.depth.est.not14, aes(x=daynum, y=wtemp)) + 
  ...
  geom_vline(xintercept=Jdays, colour="darkgreen", linetype = "longdash") + 
  labs(x="Date", y="Temperature °C at depths <1.5m")
p + theme_bw() + 
...
limits <- aes(ymin= ClassAbundance-ci, ymax= ClassAbundance+ci)
...
p <- p + 
  ...
  geom_point(aes(y = ClassAbundance, color = "Braus 2016 (2014)"), size = 3, alpha = 0.5) + 
  geom_errorbar(limits, color = "darkred", width=0.5) +
  geom_point(aes(y = ClassAbundance_zulkifly, color = "Zulkifly 2012 (2011)"), size = 3, alpha = 0.5)
p
```
